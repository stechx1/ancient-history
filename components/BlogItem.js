  ...
  return (
    <>
      <div onClick={handleClick} className={`${styles.BlogItemContainer} my-4`}>
        ...
        <div
          className={`${styles.image} my-2`}
          style={{ height: '50vh', backgroundImage: `url(${image})` }}
        ></div>
        <h4>
          {heading.length > 70 ? heading.slice(0, 70) : heading}
          {heading.length > 70 && '...'}
        </h4>
        <button className={`${styles.simpleButton} text-start px-0 py-2 `}>
          Read More
        </button>
        <hr />
